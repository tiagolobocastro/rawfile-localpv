...
class BTRFS(FileSystemBase):
    ...
    def create_snapshot(self, name: str):
        try:
            with TemporaryDirectory(
                ...
                try:
                    ...
                    run(
                        ...
                        default_subvol_path=$(btrfs subvolume show "{tmp_dir}" | head -1 | awk '{{print $1}}')
                        default_subvol="{tmp_dir}/$default_subvol_path"
                        mkdir -p "{snapshots_dir}"
                        btrfs subvolume snapshot -r "{tmp_dir}" "{snapshot_subvol}"
                        """,
                        check=True,
                        capture_output=True,
                        executable="bash",
            ...
            raise FileSystemCreateSnapshotError.from_exc(e, self.__filesystem__)
